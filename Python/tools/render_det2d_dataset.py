...
class GenDet2dDataset:
    ...
    def gen(self, outDir, setName, nImages, maxModelsPerImage):
        ...
        for n in range(0,nImages):
            ...
            objs_mask=self.dr.renderToImage(img,obj_list,sizes=size_list,centers=center_list)
            ...
            images_list.append(get_image_info(img,nimgs,outFileName))
            cv2.imwrite(imageDir+outFileName,img)
...
def main():
    ...
    #gen eval set
    nImagesToGen=20,maxModelsPerImage=2
    dr.gen(outDir,'eval',nImagesToGen,maxModelsPerImage)

    #gen train set
    nImagesToGen=100,maxModelsPerImage=2
